...
export async function POST(request: Request, { params }: { params: Promise<{ brandId: string }> }) {
  ...
  const { checkIsAdmin, authorizeBrandAccess, checkIsAgencyAdmin } = await import('@/lib/auth')
      ...
      if (ads.length > 0) {
        ...
        for (const ad of ads) {
          if (!recentDomains.includes(ad.domain)) {
            ...
            } catch (err) { console.warn('[brands/check] Failed to send Slack alert:', err instanceof Error ? err.message : 'Unknown error') }
          }
        }
      }
